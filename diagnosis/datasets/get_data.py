"""Download pushshift data.
   ...
   @project
     ...
     Created on 16 July, 2019 @ 02:54 PM.
...
# Define values
URLS = ["https://files.pushshift.io/reddit/comments/",
...
def main():
    ...
    for BASE_URL in URLS:
        ...
        filtered_links = [link[2:]
                          for link in raw_links if link.startswith("./")]
        ...
        if not os.path.exists(OUTPUT_DIR):
            ...
            sha256_link = re.findall(
                SHA256_LINK_RE_PATTERN, submissions_page)[0][2:]
            hash_file = (req.urlopen("%s/%s" %
                                     (BASE_URL, sha256_link)).read().decode("utf-8"))

            hash_file_pairs = ({entry.split("  ")[1]: entry.split(
                "  ")[0] for entry in hash_file.split("\n") if len(entry.split("  ")) > 1})

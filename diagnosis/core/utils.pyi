...
################################################################################################
# +--------------------------------------------------------------------------------------------+
# | DataUtils: For tokenizing, normalizing strings, loading & lookup of word embeddings.
# +--------------------------------------------------------------------------------------------+
################################################################################################
class DataUtils(metaclass=ABCMeta):
    """DataUtils: For tokenizing, normalizing strings, loading & lookup of work embeddings.

    Methods:
        @staticmethod
        def tokenize(text: str, **kwargs) -> List[str]:
            Normalize & tokenize a chunk of text into individual words.

        @staticmethod
        def normalize(s: str) -> str:
            Normalizes a given string: Lowercase, trim and remove non-letter characters.
        ...
        def load_glove(glove_file: str) -> Dict[str, np.ndarray]:
            Load GloVe embeddings into a dictionary where the word is the look-up key & value is the word vector.

        @staticmethod
        def lookup(embedding: Dict[str, np.ndarray],
                   vector: Union[List[float], Tuple[float], np.ndarray]) -> Tuple[str, np.ndarray]:
    ...
    def tokenize(text: str, **kwargs) -> List[str]:
        ...
        Args:
            text (str): String to be tokenized. Note that strings are
                first converted into ascii characters and special chars
                are stripped off before tokenization is performed.

        Keyword Args:
            See nltk.tokenize.word_tokenize(...)
    ...
    @staticmethod
    def unicode2ascii(s: AnyStr) -> str:
        """Turn a Unicode string to plain ASCII.
    ...
    @staticmethod
    def load_glove(glove_file: AnyStr) -> Dict[str, np.ndarray]:
        """Load GloVe embeddings into a dictionary where the word is the look-up key & value is the word vector.

        Args:
            glove_file (str): Path to GloVe embedding.

        Returns:
            Dict[str, np.ndarray]: Word is the look-up key while
            the value is a vector representation of the word.
        """

    @staticmethod
    def lookup(embedding: Dict[str, np.ndarray], vector: Union[Array[float], np.ndarray]) -> Tuple[str, np.ndarray]:
        """Lookup word embeddings.

        Args:
            embedding (Dict[str, np.ndarray]): Embedding-word mapping dictionary.
            vector (Union[Array[float], np.ndarray]): Vector lookup.

        Returns:
            Tuple[str, np.ndarray] - Returns word and its corresponding word vector.
        """
